...
class DilatedResNet(nn.Module):
    def __init__(self, block, model="resnet101", num_classes=3):
...
class Deeplab(nn.Module):
    def __init__(self, block=Bottleneck, num_classes=3):
        super(Deeplab, self).__init__()
        self.dilated_resnet = DilatedResNet(block=block, num_classes=num_classes)
        self.upsampl1_1 = nn.Upsample(scale_factor=0.75, mode='bilinear', align_corners=True)
        self.upsampl1_2 = nn.Upsample(scale_factor=0.5, mode='bilinear', align_corners=True)
        self.upsampl2 = nn.Upsample(scale_factor=8, mode='bilinear', align_corners=True)  
    ...
    def forward(self, x):
        ...
        x4 = torch.max(torch.max(x1, x2), x3)
